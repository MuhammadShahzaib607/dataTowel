...
import { LayoutDashboard, Package, ShoppingCart, Newspaper, Settings, Menu, X, LogOut, Users, Bell, PanelLeftClose, PanelLeftOpen } from "lucide-react";
        ...
        <aside className={`hidden lg:flex flex-col ${sidebarWidth} bg-white border-r border-[#E8E6DF]/50 sticky top-0 h-screen transition-[width] duration-300 ease-in-out`}>
              ...
              {sidebarLinks.map((link) => {
                ...
                return (
                  <li key={link.href}>
                    ...
                    {!sidebarCollapsed && link.href === "/admin/notifications" && unreadCount > 0 && (
          ...
          <div className={`${sidebarCollapsed ? "px-2" : "px-4"} mb-2`}>
            <button
              ...
              className={`flex items-center gap-3 rounded-lg text-[13px] font-medium text-[#6F6F69] hover:bg-[#F2EFE8] hover:text-[#171717] transition-all cursor-pointer w-full ${
                sidebarCollapsed ? "justify-center px-2 py-2.5" : "px-4 py-2.5"
              }`}
            >
              {sidebarCollapsed ? (
